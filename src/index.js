import { request, sleep, linkEndpoint } from "./controladoUtils.js";
...
const firstPlayableChampionsDropdown = new Dropdown(
  "1st pick",
  "controladoPick",
  0,
  getPlayableChampions,
);
const secondPlayableChampionsDropdown = new Dropdown(
  "2nd pick",
  "controladoPick",
  1,
  getPlayableChampions,
...
const firstAllChampionsDropdown = new Dropdown(
  "1st ban",
  "controladoBan",
  0,
  getAllChampions,
);
const secondAllChampionsDropdown = new Dropdown(
  "2nd ban",
  "controladoBan",
  1,
  getAllChampions,
...
async function onReadyCheck() {
  if (autoAcceptCheckbox.config.enabled === true) {
    console.debug(
      "auto-champion-select(auto-accept): Ready check detected, accepting in 2 seconds...",
    );
    await sleep(2000);
    await autoAccept();
  }
...
async function autoAccept() {
  ...
  if (response.ok) {
    ...
    console.error(
      "auto-champion-select(auto-accept): Failed to accept ready check",
      response,
    );
  }
}
...
window.addEventListener("load", async () => {
  ...
  checkboxesContainer.append(
    autoAcceptCheckbox.element,
    pickCheckbox.element,
    banCheckbox.element,
  );
  dropdownsContainer.append(
    firstPlayableChampionsDropdown.element,
    secondPlayableChampionsDropdown.element,
  );
  dropdownsContainer.append(
    firstAllChampionsDropdown.element,
    secondAllChampionsDropdown.element,
  );

  const pluginSection = new SocialSection(
    "Auto champion select",
    dropdownsContainer,
    checkboxesContainer,
  );
  socialContainer.append(
    pluginSection.element,
    checkboxesContainer,
    dropdownsContainer,
  );

  console.debug(
    `auto-champion-select(${version}): Report bugs to Balaclava#1912`,
  );
});
